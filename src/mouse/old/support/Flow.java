...
public class Flow {
    ...
    public Flow(FlowTemplates template) {
        buckets = normalizeBuckets(template.flow);
    }
    ...
    public double getStepSize(double distance, int steps, double completion) {
        ...
        final double completionStep = 1d / steps;
        // TODO: Fix maybe? System.out.printf("comp=%.2f compStep=%.2f%n", completion,completionStep);
        // Define the first bucket we read from
        final double bucketFrom = (completion * (buckets.length-1));
        // Define the last bucket we read from
        final double bucketUntil = (Math.min(1, completion + completionStep) * (buckets.length-1));

        final double bucketContents = getBucketsContents(bucketFrom, bucketUntil);
        ...
        final double distancePerBucketContent = distance / (buckets.length * AVERAGE_BUCKET_VALUE);
     ...
     * from first or last bucket is just a fragment of it's full value, depending on how
    ...
    private double getBucketsContents(double bucketFrom, double bucketUntil) {
        ...
        if (buckets.length < (int) Math.max(bucketFrom, bucketUntil)) throw new RuntimeException();

        final double startMultiplier = 1 - (bucketFrom - (int) bucketFrom);
        final double endMultiplier = bucketUntil - (int) bucketUntil;
        if ((int) bucketFrom == (int) bucketUntil) sum += buckets[(int) bucketFrom] * (endMultiplier - startMultiplier);
        else sum += buckets[(int) bucketFrom] * startMultiplier + buckets[(int) bucketUntil] * endMultiplier;

        for (int i = (int) bucketFrom + 1; i < (int) bucketUntil; i++) {
            sum += buckets[i];
        }
